...
namespace signalr
{
    ...
    hub_connection_builder& hub_connection_builder::with_hub_name(const std::string& hub_name)
    {
        nlohmann::json j = nlohmann::json::array();
        j.push_back(nlohmann::json::object({ {"Name", hub_name} }));
        
        return this->with_query_string("connectionData", j.dump());
    } 
    hub_connection_builder& hub_connection_builder::with_query_string(const std::string& key, const std::string& value)
    {
    ...
    hub_connection hub_connection_builder::build()
    {
        ...
        return hub_connection(m_url, std::move(hub_protocol), m_query_string, m_log_level, m_logger, m_http_client_factory, m_websocket_factory, m_skip_negotiation);
    }
}