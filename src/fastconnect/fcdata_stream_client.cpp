#include "fastconnect/fcdata_stream_client.h"
...
namespace fastconnect
{
    namespace fcdata
    {
        ...
        void stream_client::start()
        {
            ...
            m_proxy = std::make_shared<signalr::hub_proxy>(m_connection->create_hub_proxy(U("FcMarketDataV2Hub")));
            ...
            if(m_connection->get_connection_state() == signalr::connection_state::connected){
               auto args = web::json::value::array();
                args[0] = web::json::value::string(utility::conversions::to_string_t(m_channel));
                m_proxy->invoke<void>(utility::conversions::to_string_t("SwitchChannels"), args).wait();
            }
        }
        void stream_client::subscribe(const std::string& channel, std::function<void(const std::string &)> callback){
            m_channel = channel;
            m_callback = callback;
        }
    }
